...
import { UtensilsCrossed, LogOut, Clock, MapPin, Search, Receipt, CheckCircle2, ChevronRight, X } from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: orders = [], isLoading } = useQuery({
    ...
    queryFn: () => selectedRestaurant ? api.orders.listByRestaurant(selectedRestaurant.id) : Promise.resolve([]),
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: (updatedOrder) => {
      ...
      toast({
        ...
        description: `Order #${updatedOrder.id.slice(0, 8)} marked as ${updatedOrder.status.replace("_", " ")}`,
      });
      if (updatedOrder.status === 'ready' || updatedOrder.status === 'picked_up' || updatedOrder.status === 'delivered') {
  ...
  if (!selectedRestaurant) {
    return null; // Will show picker from context
  }

  const pendingOrders = orders.filter(o => o.status === "pending").sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
  const preparingOrders = orders.filter(o => o.status === "preparing").sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
  const readyOrders = orders.filter(o => o.status === "ready").sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()); // newest first
      ...
      <aside className="w-64 bg-sidebar text-sidebar-foreground flex-shrink-0 flex flex-col border-r border-sidebar-border">
        <div className="p-6 border-b border-sidebar-border">
          <div className="flex items-center gap-2 mb-6 text-sidebar-primary">
            ...
            <h1 className="font-bold text-xl tracking-tight text-sidebar-primary-foreground">Jatek</h1>
          </div>
          <div>
            <p className="text-xs text-sidebar-accent-foreground font-medium uppercase tracking-wider mb-1">Restaurant</p>
            <p className="font-semibold truncate">{selectedRestaurant.name}</p>
          </div>
        </div>

        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          <Button variant="secondary" className="w-full justify-start bg-sidebar-accent text-sidebar-accent-foreground hover:bg-sidebar-accent/90" data-testid="nav-orders">
          ...
          {selectedOrder && (
            <>
              <div className="p-6 pb-4 border-b bg-muted/10">
                <div className="flex items-center justify-between mb-4">
                  <Badge variant={
                    ...
                    selectedOrder.status === 'ready' ? 'secondary' : 'outline'
                ...
                <SheetTitle className="text-2xl mb-1">Order #{selectedOrder.id.slice(0, 8)}</SheetTitle>
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                      {selectedOrder.items?.map(item => (
                        <div key={item.id} className="flex justify-between items-start text-sm">
                          ...
                          <span className="text-muted-foreground">${((item.menuItem?.price || 0) * item.quantity).toFixed(2)}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="bg-muted p-3 rounded-md text-sm">
                      <p className="font-medium mb-1">{selectedOrder.customer?.name}</p>
                      <p className="text-muted-foreground leading-relaxed">{selectedOrder.deliveryAddress}</p>
                    </div>
                  </div>
              ...
              <div className="p-6 border-t bg-background mt-auto">
                ...
                {(selectedOrder.status === 'ready' || selectedOrder.status === 'picked_up' || selectedOrder.status === 'delivered') && (
                  <Button 
                    className="w-full h-12 text-base" 
                    variant="outline"
          ...
          {isLoading ? (
            ...
            orders.map(order => {
              ...
              return (
                <Card 
                  key={order.id} 
                  className={`cursor-pointer hover:border-primary/50 transition-colors shadow-sm ${isUrgent ? 'border-destructive/50 shadow-destructive/10' : ''}`}
                  onClick={() => onSelect(order.id)}
                  data-testid={`order-card-${order.id}`}
                >
                  <div className="p-4">
                    <div className="flex justify-between items-start mb-2">
                      <span className="font-mono text-sm font-semibold text-primary">#{order.id.slice(0, 6)}</span>
                      <div className={`text-xs font-medium flex items-center gap-1 ${isUrgent ? 'text-destructive' : 'text-muted-foreground'}`}>
                        <Clock className="w-3 h-3" />
                        {formatDistanceToNow(new Date(order.createdAt), { addSuffix: true })}
                      </div>
                    </div>
                    
                    <h4 className="font-semibold text-base mb-1 truncate">{order.customer?.name || "Guest"}</h4>
