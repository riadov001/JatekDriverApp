...
export function PromotionsCarousel() {
  ...
  const promos = useQuery({ queryKey: ["promotions"], queryFn: getPromotions });

  if (!promos.data?.length) return null;
  ...
  return (
    <View style={styles.wrap}>
      ...
      >
        {promos.data.map((p) => (
          <PromoCard key={p.id} p={p} colors={colors} />
        ))}
      </ScrollView>
    </View>
  );
...
function PromoCard({
  ...
  const pct = Math.min(100, (p.progress / Math.max(1, p.required)) * 100);
  return (
    <View
      style={[
        styles.card,
