...
export function ActiveOrderProvider({
  ...
  const beginTracking = useCallback(
    async (orderId: string): Promise<string | null> => {
      ...
      if (!perm.granted || !perm.background) {
        return (
          perm.message ??
          "Permission de localisation \"Toujours\" requise pour démarrer la course."
        );
      }
      try {
        // Make sure both WS clients are up before we go live
        wsRef.current.connect().catch(() => {});
        getOrderTrackingClient().connect().catch(() => {});
        await startActiveOrderTracking(orderId);
        setActiveOrderId(orderId);
        setTrackingActive(true);
        console.log(`[active-order] tracking started for ${orderId}`);
        return null;
      } catch (e) {
  ...
  const endTracking = useCallback(async () => {
    ...
    console.log("[active-order] tracking stopped");
  }, []);
  ...
  return (
    <ActiveOrderCtx.Provider value={value}>{children}</ActiveOrderCtx.Provider>
  );
}
